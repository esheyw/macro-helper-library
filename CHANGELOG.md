...
## Version 0.3.0
- Gone system-agnostic! New module, new module ID, unfortunately that means uninstalling the old one and installing this one manually.
  ...
  - Added Global Access setting to put the API in global scope as `mhl`, defaults to on.
  - If neither of the above options are enabled, the API can always be accessed via `game.modules.get('pf2e-module-helper-library').api`. This is recommended for anyone using MHL as a module dependency.
...
- Implemented MHLSettingsManager class
  ...
  - Handling for setting groups
  - Handling for any hooks conditional on settings
  - Option to automatically make 'reset to default' buttons per setting, group, and/or module.
  ...
  - Basic color picker support (only allows `#FFFFFF` format due to browser limitations)
...
- ***Breaking*** Overhauled error handling functions, see documentation for new specifics
  ...
  - Imlpemented the Log Level setting, defaults to Debug. Determines what level `mhlog` calls, without a defined type and made after the setup hook, use. Calls made before Setup default to Error.
